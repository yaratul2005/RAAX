...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::create('mushak_6_3s', function (Blueprint $table) {
            $table->id();
            $table->uuid('tenant_id');
            $table->date('issue_date');
            $table->string('buyer_name');
            $table->string('buyer_bin')->nullable();
            $table->bigInteger('total_value')->comment('Stored in cents');
            $table->bigInteger('total_vat')->comment('Stored in cents');
            $table->char('currency_code', 3);
            $table->timestamps();
            $table->softDeletes();
        });

        $tables = ['mushak_4_3s', 'mushak_6_1s', 'mushak_6_2s', 'mushak_6_3s'];
    ...
    {
        Schema::dropIfExists('mushak_6_3s');
